/**
 * Created by blagrone on 6/6/16.
 */
angular.module('scheduleAdvisor',['ngRoute', 'ui.bootstrap', 'ngAnimate','ngCookies','ngTouch'])
    .controller('clinicMigrationController',function($scope,APIService,buildChartService){
        $scope.migrations = {}
        var temp1;
        var temp2;

        $scope.windowHeight = $(window).height() - 8;
        $scope.windowWidth = $(window).width() - 8;

        APIService.getLocations().then(function(response){
            $scope.locations = response.data;
            for(i=0;i<foo.items.length;i++){
                var tempData = {};
                if(temp1!=foo.items[i].from_addr){
                    temp2 = foo.items[i].from_addr;
                    $scope.migrations[foo.items[i].from_addr] = {};
                    for(j=0;j<foo.items.length;j++){
                        if(temp2===foo.items[j].from_addr){
                            tempData[foo.items[j].to_addr] = foo.items[j].distinct_encounter_cnt;
                        }
                    }
                    $scope.migrations[foo.items[i].from_addr] = tempData;
                } else {
                }
                temp1 = foo.items[i].from_addr;
            }
            var states = $scope.locations;
            for (var prop in states){
                for (var prop2 in $scope.migrations){
                    //start cycling through group
                    var testVar = 0;
                    for (prop3 in $scope.migrations[prop2]){
                        if(states[prop]===prop3)
                            testVar = 1;
                    }
                    if (testVar === 0)
                        $scope.migrations[prop2][states[prop]]= 0;
                }
            }
            buildChartService.modCSS($scope.windowHeight,$scope.windowWidth)
            buildChartService.travelChart(states,$scope.migrations,$scope.windowHeight,$scope.windowWidth);
        });
    })
    .controller('clinicMigrationMapController',function($scope,APIService){
        var map;
        function initMap() {
    ...
    .service('buildLocationObject',function(){
        var dataObject = {};

        dataObject.locationTravel = function(){

        };

        return dataObject;
    })
    .service('buildChartService',function(){
        var chartBuilder = {}

        chartBuilder.modCSS = function(windowHeight,windowWidth){
        ...
        chartBuilder.travelChart = function(statename,migration,windowHeight,windowWidth){
            ...
            function mouseover_group(d, i) {
                d3.select("#tooltip")
                    ...
                    .html(chartBuilder.groupTip(d,states,topstates,migration))
            ...
            function mouseover_chord(d, i) {
                ...
                d3.select("#tooltip")
                    .style("visibility", "visible")
                    //.html(chordTip(d))
                    .html(chartBuilder.chordTip(d,states,migration))
                    .style("top", function () { return (d3.max([50,d3.event.pageY - 80]))+"px"})
            ...
            /*function chordTip (d) {
                ...
                if (move1to2 > move2to1) {
                    return q(move1to2) + " patients travelled from " + stusab1 + " to " + stusab2 + "<br/>"
                        + q(move2to1) + " patients travelled from " + stusab2 + " to " + stusab1;
                }
                else {
                    return q(move2to1) + " patients travelled from " + stusab2 + " to " + stusab1 + "<br/>"
                        + q(move1to2) + " patients travelled from " + stusab1 + " to " + stusab2;
                }
            };*/
        ...
        chartBuilder.chordTip = function(d,states,migration){
            var p = d3.format(".1%"), q = d3.format(",");
            var stusab1 = states[d.target.index];
            var stusab2 = states[d.source.index];
            var move1to2 = migration[stusab2][stusab1];
        ...
        chartBuilder.groupTip = function(d,states,topstates,migration){
            var p = d3.format(".1%"), q = d3.format(",");
            var stusab = states[d.index];
            var inmig = topstates[stusab]["in"];
            var outmig = topstates[stusab]["out"];
    ...
    .factory('APIService', function($http){
        ...
        APIData.getLocations = function () {
            return $http({
                url:'content/data/locations.json',
                method:"GET"
            });
    ...
    .config(function($routeProvider,$locationProvider){
        $routeProvider
            .when('/clinicmigration',{
                templateUrl: './content/templates/clinicmigration.html',
                controller: 'clinicMigrationController',
                title: 'Clinic Migration'
            })
            .when('/clinicmigrationmap',{
                templateUrl: './content/templates/clinicmigrationmap.html',
                controller: 'clinicMigrationMapController',
                title: 'Clinic Migration Map'
            })
            .otherwise({
                redirectTo: '/clinicmigration'
            });
    });